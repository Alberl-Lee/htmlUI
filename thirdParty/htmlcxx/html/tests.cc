 ...
 * 
 * Copyright 2005-2010 Davi de Castro Reis and Robson Braga Ara渏o
...
class HtmlTest {
	...
	bool parse() {
		...
		parser.parse(html);
		tr = parser.getTree();
		//cerr << tr << endl; // Node重载了<<，但tree没有，要自己遍历输出
		cerr << " ok" << endl;
		return true;
	}
	...
	bool string_manip() {
		...
		myassert(HTML::decode_entities(entities) == "nos somos do clube atletico mineiro ??brasil   &teste; 沅 &aacute ?&end   ");
		...
		myassert(HTML::convert_link(link3, root_link) == "http://www.akwan.com.br/teste/servi鏾.html");
...
class CharsetTest
{
	public:
		void test()
		{
			...
			//myassert(cc.convert("Voc锚 茅 o meu visitante n煤mero") == "Voc??o meu visitante n鷐ero");

			//CharsetConverter cc2("ISO-8859-1", "UTF8");
			//myassert(cc2.convert("Voc??o meu visitante n鷐ero") == "Voc锚 茅 o meu visitante n煤mero");
		}
};
...
int main_htmlcxx(int argc, char **argv) {
